...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, patch_sizes=7, stride=4, in_channels=3, embed_dim=768):
        super().__init__()
        patch_sizes = to_2tuple(patch_sizes)
        self.patch_sizes = patch_sizes
        self.proj = nn.Conv2d(in_channels, embed_dim, kernel_size=patch_sizes, stride=stride,
                              padding=(patch_sizes[0] // 2, patch_sizes[1] // 2))
...
class lvt(nn.Module):
    def __init__(self, num_layers=[2, 2, 2, 2], in_channels=3, num_classes=1000, patch_sizes=4,
                 ...
                 num_stages=4, strides=None, out_indices=None, mlp_ratio=None):
        ...
        for stage_idx in range(len(self.layers)):
            _patch_embed = OverlapPatchEmbed(
                patch_sizes=7 if stage_idx == 0 else 3,
